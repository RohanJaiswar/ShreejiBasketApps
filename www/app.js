...
async function initFCM() {
    if (!window.firebase || !currentUser?.id) return;
    try {
        if (!firebase.apps.length) firebase.initializeApp(FIREBASE_CONFIG);

        // Register the background handler service worker
        ...
        // Save token to vendor row in Supabase
        await sb.from('vendors').update({ fcm_token: token }).eq('id', currentUser.id);
        currentUser.fcm_token = token;
...
function enterHome() {
    ...
    startNotifRealtime();      // live push via Supabase Realtime
    startNotifPolling();       // fallback 30s poll
    initFCM();                 // register FCM push token (Android/PWA)
    lucide.createIcons();
}
...
function getNotifColors(type) {
    const map = {
        order:   { icon: 'package',      bg: 'bg-blue-50 dark:bg-blue-900/20',   text: 'text-blue-500',   border: 'border-blue-100 dark:border-blue-800/30' },
        payment: { icon: 'banknote',     bg: 'bg-green-50 dark:bg-green-900/20', text: 'text-green-500',  border: 'border-green-100 dark:border-green-800/30' },
        warning: { icon: 'alert-triangle', bg: 'bg-yellow-50 dark:bg-yellow-900/20', text: 'text-yellow-500', border: 'border-yellow-100 dark:border-yellow-800/30' },
        promo:   { icon: 'tag',          bg: 'bg-purple-50 dark:bg-purple-900/20', text: 'text-purple-500', border: 'border-purple-100 dark:border-purple-800/30' },
        info:    { icon: 'bell',         bg: 'bg-slate-50 dark:bg-slate-800',    text: 'text-slate-500',  border: 'border-slate-200 dark:border-slate-700' },
    };
...
function renderNotifications() {
    ...
    const unread = liveNotifications.filter(n => !n.is_read).length;
    ...
    if (!liveNotifications.length) {
        ...
        <div class="flex flex-col items-center justify-center py-10 text-slate-400">
            <i data-lucide="bell-off" class="w-10 h-10 mb-3 opacity-40"></i>
    ...
    list.innerHTML = liveNotifications.map(n => {
        ...
        const icon = n.icon || c.icon;
        const time = new Date(n.created_at).toLocaleString('en-IN', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' });
        return `
        <div class="flex gap-3 p-4 bg-white dark:bg-dark-surface rounded-2xl border transition-all
             ${n.is_read ? 'border-slate-100 dark:border-dark-border opacity-70' : 'border-primary-200 dark:border-primary-900 shadow-sm'}
             cursor-pointer active:scale-[0.98]" onclick="markRead('${n.id}')">
            <div class="w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center border ${c.bg} ${c.text} ${c.border}">
                <i data-lucide="${icon}" class="w-5 h-5"></i>
            </div>
            <div class="flex-1 min-w-0">
                <div class="flex justify-between items-start gap-2">
                    <h4 class="text-sm font-bold text-slate-800 dark:text-slate-100 leading-tight">${n.title}</h4>
                    ${!n.is_read ? '<span class="w-2 h-2 rounded-full bg-primary-500 flex-shrink-0 mt-1"></span>' : ''}
                </div>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 leading-relaxed">${n.message || n.body || ''}</p>
...
async function loadNotifications() {
    ...
    try {
        // Fetch notifications for this vendor OR broadcast (vendor_id IS NULL)
        const { data, error } = await sb
            .from('notifications')
            .select('*')
            .or(`vendor_id.eq.${currentUser.id},vendor_id.is.null`)
        ...
        console.warn('[Notif] Load failed:', err.message);
    }
}

function startNotifPolling() {
    stopNotifPolling();
    notifPollTimer = setInterval(loadNotifications, 30000); // every 30s
}
...
function startNotifRealtime() {
    ...
    notifChannel = sb
        .channel('vendor-notifications-' + currentUser.id)
        .on('postgres_changes', {
            event: 'INSERT',
            schema: 'public',
            table: 'notifications',
            ...
            liveNotifications.unshift(payload.new);
            renderNotifications();
            showToast(`🔔 ${payload.new.title}`, 4000);
        })
        .on('postgres_changes', {
            event: 'INSERT',
            schema: 'public',
            table: 'notifications',
            filter: `vendor_id=is.null`   // broadcast notifications
        }, payload => {
            // avoid duplicate if already in list
            if (!liveNotifications.find(n => n.id === payload.new.id)) {
                liveNotifications.unshift(payload.new);
                renderNotifications();
                showToast(`📢 ${payload.new.title}`, 4000);
            }
...
window.markRead = async function(id) {
    ...
    n.is_read = true;          // optimistic UI update
    renderNotifications();
    try {
        await sb.from('notifications').update({ is_read: true }).eq('id', id);
    } catch (err) {
        console.warn('[Notif] markRead failed:', err.message);
        n.is_read = false;     // revert on failure
        renderNotifications();
    }
}
